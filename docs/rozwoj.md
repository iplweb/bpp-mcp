...
| `src/bpp_mcp/__init__.py` (`__version__`) | kod w runtime; trafia do `serverInfo` w handshake MCP |
...
```bash
grep -rn '<stara-wersja>' pyproject.toml src/bpp_mcp/__init__.py manifest.json
```
...
odtwarzają zachowanie sprzed 0.4.0.
